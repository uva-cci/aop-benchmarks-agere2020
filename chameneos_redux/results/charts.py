...
def open_benchmark(filename, order=(0, 1)):
    global LOG_SCALE
...
def draw_3_dim(y, dimension_names, feature_names):
    ...
    for dim1 in y.keys():
        ...
        for dim2 in y[dim1].keys():
            xx.append(dim2)
            yy_mean.append(averages[dim1][dim2])
            yy_dev.append(stds[dim1][dim2])


        plt.title(label=feature_names[i] + " in function of " + dimension_names[1])
        plt.errorbar(xx, yy_mean, yerr=yy_dev,
                     label=dimension_names[0] + " " + dim1, elinewidth=1,
                     capsize=0)
...
y_cpudata_mean, y_cpudata_dev, y_total_time, y_internal_time\
    = open_benchmark("benchmark-agentscript_compiled-4096-65536.csv", [0,1])
...
for i, y in enumerate([ y_total_time ]): #, y_internal_time, y_cpudata_mean]): # , y_total_time]): # , y_cpudata_mean, y_cpudata_dev]):
    ...
    averages, stds = compute_averages(y, 2)
    print(averages)
    print(stds)
